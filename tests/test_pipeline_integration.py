...
class Wiring:
    def __init__(self, tmp_path, instrument):
        ...
        runner = StrategyRunner(
            [("A", MomentumStrategy([INSTRUMENT_ID], threshold_bps=5.0, order_qty=1.0))], self.store, self.gateway
        )
...
def test_full_tick_sequence_produces_expected_fills_and_log(tmp_path, instrument):
    ...
    w.bus.publish(MarkEvent(INSTRUMENT_ID, 50_030, 2))  # +6bps: momentum A buys 1
    w.bus.publish(MarkEvent(INSTRUMENT_ID, 50_000, 3))  # ~-6bps: momentum A sells 1 back to flat
...
def test_desk_kill_switch_rejects_strategy_orders(tmp_path, instrument):
    ...
    w.bus.publish(MarkEvent(INSTRUMENT_ID, 50_030, 2))  # would trigger a momentum BUY
